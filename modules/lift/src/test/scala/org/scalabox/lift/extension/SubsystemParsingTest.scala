 ...
 * @since // TODO
 */
class SubsystemParsingTest extends AbstractScalaSubsystemTest {
   ...
   @Test def testParseSubsystem {
      ...
      assert(2 === operations.size)

      // Check that each operation has the correct content
      val addSubsystem = operations.head
      assert(ADD === addSubsystem.get(OP).asString)
      var addr = PathAddress.pathAddress(addSubsystem.get(OP_ADDR))
      assert(1 === addr.size)
      var element = addr.getElement(0)
      assert(SUBSYSTEM === element.getKey)
      assert(SubsystemExtension.SUBSYSTEM_NAME === element.getValue)
   ...
   @Test def testInstallIntoController {
      ...
      assert(model.get(SUBSYSTEM).hasDefined(SubsystemExtension.SUBSYSTEM_NAME))
      assert(model.get(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME).hasDefined("type"))
      assert(model.get(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME, "type").hasDefined("tst"))
      assert(model.get(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME, "type", "tst").hasDefined("tick"))
      assert(12345 === model.get(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME, "type", "tst", "tick").asLong)
   }
   ...
   @Test def testDescribeHandler {
      val subsystemXml =
         <subsystem xmlns={SubsystemExtension.NAMESPACE}></subsystem>
   ...
   @Test def testExecuteOperations {
      ...
      val services = super.installInController(subsystemXml)
      val fooTypeAddr = PathAddress.pathAddress(PathElement.pathElement(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME), PathElement.pathElement("type", "foo"))
      ...
      assert(model.get(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME, "type", "tst").hasDefined("tick"))
      assert(12345 === model.get(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME, "type", "tst", "tick").asLong)
      assert(model.get(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME, "type").hasDefined("foo"))
      assert(model.get(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME, "type", "foo").hasDefined("tick"))
      assert(1000 === model.get(SUBSYSTEM, SubsystemExtension.SUBSYSTEM_NAME, "type", "foo", "tick").asLong)
