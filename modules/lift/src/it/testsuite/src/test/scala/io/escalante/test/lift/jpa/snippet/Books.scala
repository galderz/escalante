...
package io.escalante.test.lift.jpa.snippet
...
import io.escalante.test.lift.jpa.{Author, Book, Model, snippet}
...
class Books {
  ...
  def add = {
    ...
    val default = Full(snippet.Authors.authorVar.is.id.toString)
    
    "type=hidden" #> SHtml.hidden(() => bookVar(current)) &
    "name=title" #> SHtml.text(book.title, book.title = _) &
    "name=published" #> SHtml.text(formatter.format(book.published),
        v => book.published = formatter.parse(v)) &
    "#author" #> SHtml.select(choices, default, { authId : String =>
      book.author = Model.getReference(classOf[Author], authId.toLong)}) &
