...
class ArtifactSubsystemParser
  ...
  override def writeContent(writer: XMLExtendedStreamWriter,
    ...
    debug("Write Artifact subsystem")

    // Write out the main subsystem element
    ctx.startSubsystemElement(
      ArtifactExtension.ARTIFACT_SUBSYSTEM_NAMESPACE, false)
  ...
  override def readElement(reader: XMLExtendedStreamReader, ops: java.util.List[ModelNode]) {
    ...
    val addSubsystemOp = ArtifactExtension.createAddSubsystemOperation
